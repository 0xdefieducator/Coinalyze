...
# Save this file as `Open_Interest_History.py` and run it with
...
EXCHANGE_CODES = {
    "A": "binance", "B": "bitstamp", "C": "coinbase", "D": "bitforex",
    "E": "mercadobitcoin", "F": "bitfinex", "G": "gemini", "H": "hyperliquid",
    "I": "bit2c", "J": "luno", "K": "kraken", "L": "bitflyer",
    "M": "btcmarkets", "N": "independentreserve", "P": "poloniex", "U": "bithumb",
    "V": "vertex", "W": "woo", "Y": "gateio", "0": "bitmex",
    "2": "deribit", "3": "okx", "4": "huobi", "6": "bybit",
    "7": "phemex", "8": "dydx"
}

symbols = [
    "BTCUSDT_PERP.A", "BTCUSDC_PERP.A", "BTCUSD_PERP.0", "BTCUSDT_PERP.0",
    "BTCUSDT_PERP.F", "BTCUSD_PERP.F", "BTCUSDT.6", "BTCUSD.6",
    "BTCUSDT_PERP.4", "BTCUSD_PERP.4", "BTCUSDT_PERP.3", "BTCUSDC_PERP.3",
    "BTCUSD_PERP.3", "PERP_BTC_USDT.W", "BTC_USDT.Y", "BTC.H"
]

dAPI_KEY = "ceece553-3e96-4695-8618-4a9bdc78edd4"  # Replace with your actual API key
BASE_URL = "https://api.coinalyze.net/v1"
HEADERS = {"api_key": API_KEY}
...
# FETCH FUNCTION
def get_open_interest_history(symbols, interval, from_ts, to_ts):
...
if __name__ == "__main__":
    ...
    output_file = "BTC_Open_Interest_History.csv"

    print(f"📡 Fetching open interest history for: {symbols}")
    data = get_open_interest_history(symbols, interval, from_ts, to_ts)
    ...
    with open(output_file, mode="w", newline="") as file:
        ...
        for market in data:
            ...
            for row in market["history"]:
                date = time.strftime("%Y-%m-%d", time.localtime(row["t"]))
                oi = row.get("v", "NA")
